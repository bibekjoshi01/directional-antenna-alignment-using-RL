...
Figure~\ref{fig:esp32_block_diagram} highlights why the ESP32-S3 is suitable as the
central node of the proposed system. The diagram shows that USB-to-UART programming,
on-chip power regulation, wireless communication, and general peripheral interfacing are
all integrated around the main module. For this project, that integration reduces the
need for additional support circuitry and allows a single controller to handle RSSI
acquisition, actuator control, and serial debugging within a compact embedded platform.
...
Figure~\ref{fig:stepper_motor} shows the mechanical outline and mounting dimensions of
the NEMA 17 motor. This is important as a specification reference and also for
mechanical integration of the antenna platform, since the shaft size, body dimensions,
and mounting-hole pattern determine how the motor can be fixed to the rotating base and
how reliably azimuth torque can be transmitted to the antenna assembly.
...
Figure~\ref{fig:mg90s_servo} provides the dimensional constraints of the selected tilt
actuator. For this project, the compact form factor is significant because the servo is
mounted on the moving structure together with the antenna and controller hardware. The
small size and standard horn geometry make it easier to achieve a lightweight tilt
mechanism without adding excessive inertia that could degrade positioning stability.

The MG90S operates on a 4.8-6 V DC supply and draws a stall current of up to 650 mA. 
Control is achieved via standard PWM pulses from the ESP32-S3, which allows direct 
positioning without additional drivers. A decoupling capacitor of 100 nF is placed 
across the power lines to filter inductive noise and prevent transient voltage spikes 
from affecting the microcontroller logic.
...
Figure~\ref{fig:stepper_motor_driver} explains the
signal flow between the controller and the azimuth actuator. The diagram shows that the
microcontroller only needs low-complexity control signals such as STEP, DIR, and mode
selection, while the A4988 handles the higher-current phase driving required by the
motor. This separation is valuable in the present system because it keeps the ESP32-S3
interface simple and allows precise motor motion without burdening the controller with
direct coil commutation.
...
\end{itemize}
These characteristics are ideal for RSSI-driven control and reinforcement learning, as they produce predictable signal gradients and reduce learning instability. 
The antenna's~low~mass ($\approx$19--40~g) allows reliable mounting on small stepper actuators with minimal mechanical inertia.
...
Figure~\ref{fig:pcb_antenna} illustrates both the physical antenna geometry and its
directional radiation behavior. The 3D radiation pattern confirms the presence of a
dominant main lobe, which is essential for RSSI-based alignment because antenna rotation
produces a measurable change in received signal strength. This the property
that enables the controller, whether baseline or RL-based, to infer better orientations
from RSSI feedback.
...
For bench-top testing, a regulated 12V / 3A DC power supply is used to power the stepper motor through the A4988 motor driver. A separate 5V regulated rail supplies the ESP32-S3, either via an onboard linear regulator or an external buck converter. 
