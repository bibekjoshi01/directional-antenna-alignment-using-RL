...
scan while maintaining a final RSSI close to the exhaustive reference and stronger
average performance than hill-climbing. Exhaustive scan retained the best final RSSI and perfect
...
\hline
\endhead
Exhaustive scan & $-30.20$ & $-48.95$ & $-18.75$ & 67.68 s & 54.20 s & $-19.9\%$ \\
\hline
Hill-climbing & $-30.13$ & $-52.73$ & $-22.60$ & 6.51 s & 11.84 s & $+81.9\%$ \\
\hline
RL (Q-learning) & $-29.82$ & $-49.55$ & $-19.73$ & 5.51 s & 9.62 s & $+74.6\%$ \\
\hline
\end{longtable}


The absolute RSSI values in practice are lower than in simulation for all methods,
which reflects a clear simulation-to-real gap. In the practical setup, antenna
misalignment and radiation-pattern mismatch, RF losses in cables, connectors, and the
slip-ring interface, limited resolution and fluctuation in ESP32 packet-level RSSI
reporting, and environmental noise due to multipath, obstruction, and interference all
contribute to weaker and more variable measurements than those produced by the simplified
simulator. RL still transferred more favorably than hill-climbing in the sense that it
preserved a stronger final RSSI and better repeatability, but the practical measurements
also show that its performance remained below the exhaustive reference, which is
consistent with a time-efficient search.
...
Although the RL controller performed well in the tested scenarios, the results also
highlight several limitations of the adopted tabular Q-learning approach.

\textbf{Dependence on discretization:}
The deployed controller relies on discrete pan, tilt, and RSSI-trend states. This keeps
the Q-table small enough for the ESP32-S3, but it also means that the learned policy can
only choose from a limited set of orientations and may miss a better alignment that lies
between the sampled angles.

\textbf{Generalization limits:}
Because the controller uses a fixed offline-trained Q-table, it does not generalize as
flexibly as a continuously adaptive or higher-capacity learning method. Good performance
in the tested indoor and outdoor scenarios does not guarantee the same behavior under
different antenna hardware, search ranges, motion resolution, or channel conditions.

\textbf{Sensitivity to the training environment:}
The learned policy reflects the simplified simulator and training settings used during
offline learning. When the real environment differs through stronger multipath,
additional attenuation, RSSI fluctuation, or mechanical error, the practical policy can
deviate from the behavior suggested by simulation, which is visible in the reduced RSSI
and longer runtime observed on hardware.
...
Among these sources, environmental multipath, RF losses, RSSI measurement limitations,
and actuator non-idealities had the greatest influence on deviation from expected
values. They are also the main reason why hill-climbing performed poorly in the indoor
scenarios. The RL controller was less affected because it does not rely only on the
instantaneous local gradient of RSSI, but these same factors still reduced its practical
performance relative to simulation.

\subsection{Overall Analysis}

From an engineering perspective, the outputs of this chapter show that the proposed
approach is feasible in both simulation and practical deployment. The compact Q-table
could be deployed on the ESP32-S3, and the real system converged reliably in the tested
scenarios. Within this evaluation, the RL controller provided the best overall trade-off
between convergence time, final RSSI, and repeatability among the compared methods.

The practical results are important because they show that the system did not rely on
simulation alone. At the same time, the measured simulation-to-real gap and the
limitations of the tabular RL formulation indicate that the reported performance should
be interpreted within the tested hardware setup and operating conditions.
